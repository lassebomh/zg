...
<script lang="ts">
  ...
  import { abortSignal, assert, fail, now } from "./lib/utils.js";
  ...
  onMount(() => {
    ...
    const ctx = gameCanvas.getContext("2d") ?? fail();

    flushCanvas = () => {
      const ptr = js_get_image_ptr();
      const width = js_get_image_width();
      const height = js_get_image_height();
      const memory = instance.exports.memory as WebAssembly.Memory;
      ctx.canvas.width = width;
      ctx.canvas.height = height;

      const imageData = new ImageData(new Uint8ClampedArray(memory.buffer, ptr, width * height * 4), width, height);
